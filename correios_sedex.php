 ...
 *
 * @version $Id: correios_sedex.php 3220 2011-05-12 20:09:14Z Luizwbr $
 * @package VirtueMart
...
class plgVmShipmentCorreios_Sedex extends vmPSPlugin {
        ...
        if ($method->FreteProduto_SN == 1) {
            ...
            foreach ($cart->products as $k => $product) {
                ...
                if ($method->VolumeProduto_SN == '1') {
                    // pegar sempre a maior altura e a mesma dimensão
                ...
                } elseif ($method->VolumeProduto_SN == '2') {
                    // somando todos os volumes do produto                    
                    $product_height = round($product_height * $product->quantity,2);
                    if( $product_height > $this->Order_Height){
                        $this->Order_Height += $product_height;
                    }

                    $product_width = round($product_width * $product->quantity,2);
                    if( $product_width > $this->Order_Width){ 
                        $this->Order_Width += $product_width;
                    }

                    $product_length = round($product_length * $product->quantity,2);
                    if( $product_length > $this->Order_Length){ 
                        $this->Order_Length += $product_length;
                    }                    

                } else {
                    // sempre o maior volume do produto
                    if( $product_height > $this->Order_Height){
                        $this->Order_Height = $product_height;
                    }
        ...
        if ($pluginSalesPrice) {
            ...
            if ($method->MostrarPrazoEntrega_SN == '1') {
                if (!empty($this->correios_prazo) or $this->correios_prazo == '') {
                    $printPrazo = '<span class="prazo">( Entrega Aprox. ' . $this->correios_prazo . ' '.$prazoDiasCorreios.' )</span>';
                }
            }

            $pesoCorreios = '';
            if ($method->MostrarPeso_SN == '1') {
                $pesoCorreios = '<span class="peso">( '. $this->Order_WeightKG . 'Kg )</span>';
            }

            $costDisplay ='<span class="' . @$this->_type . '_cost"> (' . JText::_('COM_VIRTUEMART_PLUGIN_COST_DISPLAY').'<span class="preco">'.$costDisplay . '</span>) '.$pesoCorreios.' '.$printPrazo.'</span>';
        } else {
    ...
    function _parametrosCorreios($total_preco, $method, $peso) {
        ...
        if ($method->ValorDeclara_SN) {
            $workstring.='&nVlValorDeclarado=' . number_format($total_preco, 2, ',', '.');
        } else {
        ...
        $cache->setCaching(1);
        $dados_frete = $cache->call( array( $this, '_xmlCorreios' ),$url_busca );
        */
        return $dados_frete;
    }
    ...
    protected function checkConditions($cart, $method, $cart_prices) {
        ...
        $address = ((isset($cart->ST['zip']) and !empty($cart->ST['zip']) and strlen($cart->ST['zip']) > 8)? $cart->ST : $cart->BT);
        $this->cepDestino = trim($address["zip"]);
        ...
        $CepStart = $method->CepStart_SN;
        // $CepEnd = preg_replace('/[^0-9]/', '', $method->CepEnd_SN);
        $CepEnd = $method->CepEnd_SN;
        ...
        if (($this->Order_Length > 105 or $this->Order_Width > 105 or $this->Order_Height > 105) or ($this->Order_Length + $this->Order_Width + $this->Order_Height > 200)  )  {
            if ($method->MensagemErro_SN )
                $mainframe->enqueueMessage("Correios erro: a dimensão excede os limites dos Correios.");
            return false;
        }
